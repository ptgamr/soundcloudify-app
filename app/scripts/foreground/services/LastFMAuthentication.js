(function(){
    ...
    function LastFMAuthentication($http, Messaging){
        ...
        chrome.storage.local.get('lastfm.sessionKey', function (result) {
            _sessionKey = result['lastfm.sessionKey'];
        });

        chrome.storage.local.get('lastfm.token', function (result) {
            _token = result['lastfm.token'];
        });
            ...
            if (_token) {
                ...
                $http({
                    ...
                    if (data.session && data.session.key) {
                        ...
                        chrome.storage.local.set({'lastfm.sessionKey': data.session.key});
                        chrome.storage.local.set({'lastfm.token': ''});
        ...
        function _requestToken() {
            ...
            $http({
                ...
                if (data.token) {
                    ...
                    chrome.storage.local.set({'lastfm.token': data.token});
                    _token = data.token;
